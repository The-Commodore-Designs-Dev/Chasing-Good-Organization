...
@Component({
  selector: 'submission-references-form',
...
export class ReferencesFormComponent implements OnInit {
  ...
  populateSubmission(submission: Submission) {
    const references: FormGroup = this.refFormGroup;
    ...
    const reference1String = JSON.stringify(this.reference1);
    const reference2String = JSON.stringify(this.reference2);  
